...
from bot.config import DATABASE_PATH, DEFAULT_API_URL, DEFAULT_MODEL, DEFAULT_PROMPT
    ...
    async def upsert_user(self, user_id: int, **kwargs: object) -> None:
        ...
        if existing is None:
            await self.db.execute(
                "INSERT INTO users (user_id, prompt, api_url, model, created_at) "
                "VALUES (?, ?, ?, ?, ?)",
                (user_id, DEFAULT_PROMPT, DEFAULT_API_URL, DEFAULT_MODEL, time.time()),
            )
        if kwargs:
